...
async function checkRecord(uuid, record) {
  ...
  const res = await axios.get(
    base + "zones/" + zone + "/dns_records/" + id,
    { headers }
  );

  if (res.data.result.content != uuid + ".cfargotunnel.com") {
    console.error("Something aint right:", res.data.result);
    return false;
  }
...
async function deleteRecord(uuid, record) {
  const domain = record.split(".").slice(-2).join(".");
  if (!(domain in cache_zones)) {
    await fetch_zones();
  }
  ...
  if (res.data.result.content != uuid + ".cfargotunnel.com") {
    console.warn("The record '" + record + "' was already overwritten, ignoring...");
    return;
  }
...
module.exports = { deleteRecord, createRecord, getTunnelName, checkRecord };
